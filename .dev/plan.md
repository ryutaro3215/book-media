...
- 決めること:
  1. **分け方** — プルダウンを2つ（下書き / 公開済み）に分けるか、
     `<optgroup>` で1つのまま区切るか、一覧そのものをリスト表示に変えるか。
     **`<optgroup>` が最も変更が小さい**が、件数が増えると結局スクロールする
  2. **既定の並び** — いまは `publishedAt` の降順。書きかけは
     **最後に触った順（mtime）**の方が探しやすい可能性がある。
     `/api/articles.json` は既に `mtimeMs` を返せる（`readArticle` が持っている）
  3. **絞り込みを付けるか** — トピックや選者で絞る、打った語で絞る。
     タグの絞り込みと同じ挙動にできる
  4. **公開済みを既定で隠すか** — 隠すと誤って公開記事を触りにくくなるが、
     直したいときに一手増える
- acceptance:
  - 下書きと公開済みが視覚的に分かれている
  - 記事が50本を超えても、目的の記事に**数秒で辿り着ける**
  - 公開済みの記事を開いたとき、**下書きに戻らない**（T26 で入れた挙動を壊さない）
  - `npm run verify` が通り、本番ビルドに混入しない
- depends: T26
