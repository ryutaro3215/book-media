...
- **実装前に方針を決めること（このタスクの前半）**:
  1. **形態** — Astro の dev サーバー内に作る（`/admin` 等）か、独立した小さなアプリか。
     前者は既存のスキーマ・データをそのまま使えるが、**本番ビルドに混入しない仕組み**が要る
  2. **保存方法** — ファイルへ直接書くか、クリップボードにコピーするか。
     直接書くなら**開発時のみ有効なAPIルート**が必要
  3. **どこまでやるか** — frontmatter の入力補助だけか、本文（Markdown）も書けるようにするか、
     プレビューまで出すか。**小さく作って足す**方針を推奨
  4. **CLI をどうするか** — 併存させるか、アプリに寄せて CLI は縮小するか
- acceptance（実装フェーズ）:
  - `npm run <何か>` で起動し、**ローカルからのみアクセスできる**
  - 大トピックを選ぶと**小トピックの候補が絞られる**
  - タグは入力で絞り込め、無ければ追加できる
  - ISBN を入れると書誌が自動取得され、**その場で修正できる**
  - 保存すると `src/content/interviews/<slug>.md` が正しい形式で作られる
  - **本番ビルドに一切混入しない**（`dist/` に出力されないことを確認する）
- depends: なし
- skills: dev-design（画面を作る場合）
