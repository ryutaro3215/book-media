...
## T31: 分野の地図を作る（/about に）

- goal: **このメディアがどんな分野・ジャンルを扱っているのかが一目で分かる**面を作る。
  読者が「自分の興味がどこにあるか」を掴めるようにする
- 背景:
  - いま分野に触れられる面は `/topics/` だけで、**記事のある大トピックを
    並べているにすぎない**。「何が扱われうるのか」の全体像は出ていない
  - **小トピック919・タグ658は、どこにも姿を見せていない。**
    ページを作らないと決めた（薄いページが量産されるため）ので、
    検索で引くしかないが、**引くには語を知っている必要がある**
  - 読者は「経営学」を探しに来るとは限らない。「働き方の本ないかな」で来る
- **現状の数字（2026-08-08）**:
  - 大トピック31のうち、記事があるのは24。**7つは記事ゼロ**
    （デザイン学 / 歯学 / 薬学 / 健康科学 / 軍事学・防衛学 / 人類学・考古学 / 地理学）
  - **公開記事があるのは4トピックだけ**（哲学・経営学・数学・心理学）。
    残り20は下書きのみ
  - `topics.json` の `description` は **31件すべて空**。
    分野を説明する文章がまだ1つも無い
- **設計上の難所（ここが本題）**:
  1. **記事ゼロの分野をどう見せるか。** 31を全部並べると、
     読者が押した先の8割が「記事なし」になる。**地図として正直だが、
     媒体としては空っぽに見える。** 逆に4つだけ出すと地図にならない
  2. **919の小トピックをどう畳むか。** 全部出せば圧倒され、
     出さなければ「何が扱われうるか」が伝わらない
  3. **説明文を誰が書くか。** 31分野の説明を書くのは重い作業。
     **書かずに名前だけ並べる地図で足りるのか**を先に決める
- 決めること:
  - **置き場所** — `/about` の中か、独立したページ（`/map` など）か。
    `/about` は「このメディアについて」を読む面で、
    **分野を探しに来る人が最初に開く面ではない**。
    `/topics/` を作り替える方が導線として素直かもしれない
  - **粒度** — 大トピックだけか、小トピックまで見せるか
  - **記事ゼロの扱い** — 出す（薄く）／出さない／「これから」と明示する
  - **説明文** — 書く／書かない／主要な分野だけ書く
- acceptance:
  - 読者が**扱う分野の全体像**を1画面で掴める
  - 記事のある分野に**そこから辿り着ける**
  - 記事が増えても壊れない（記事ゼロが減っていく方向に自然に育つ）
  - `npm run verify` が通る
- depends: なし
- skills: dev-design（画面を作る場合）
- 備考: **記事を増やすことと同時に進む話。** いま作ると7分野が空のままになる。
  「先に地図を作って埋めていく」のか「埋まってから地図にする」のかは、
  1つ目の決めること（置き場所）より前に決まっているとよい
