...
- **これは「生成」から「編集」への変更である**（段階1との決定的な違い）:
  ...
  - `scripts/lib/article-file.mjs` の `buildArticle` は**新規生成専用**。
    ...
    （保存のたびに本文が消えるのが最悪の事故）
- 決めること:
  1. **保存の衝突をどうするか** — エディタで開いたまま `/admin` で保存すると、
     エディタ側の変更が消える。**いまの段階1にも同じ穴がある**が、
     長文をブラウザで書くようになると現実に踏む。
     読み込み時のタイムスタンプを持って、変わっていたら警告する程度で足りるか
  2. **保存の仕方** — 明示的な保存ボタンか、自動保存か。
     自動保存は事故が静かに進むので、**明示保存を推奨**
  3. **入力欄** — `textarea` で足りるか、Markdown の見た目が要るか。
     導入文には見出し・引用が使える。**まず `textarea` + 文字数表示**で試すのを推奨
  4. **エディタとの併用をやめるのか** — 併用を続けるなら 1 の対策が必須。
     `/admin` に寄せるなら記事一覧と削除も要る（**削除は作らない方が安全**）
- acceptance:
