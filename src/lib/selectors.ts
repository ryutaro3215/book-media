...
export type Selector = {
  ...
  affiliation?: string;
  /**
   * 専門・関心領域（任意）。例: `経営学（組織論）／近世哲学`
   *
   * **応募フォームで最初に聞いている項目**（`docs/tally-form.md`）。
   * 「何をしている人か」（`bio`）でも「なぜ詳しいか」（`credentials`）でもなく、
   * **どの領域の人か**を一目で示すための短い一行。
   *
   * 記事の `topic` とは別物。`topic` は記事1本ごとの分野で、
   * 閉じた語彙から選ぶ。こちらは選者自身の言葉で、
   * 語彙を閉じない（「写真史」「合意形成のデザイン」のように、
   * 分類には無い言い方で自分を説明できるほうがよい）。
   */
  expertise?: string;
  /**
   * 略歴（任意）。何をしている人か。
